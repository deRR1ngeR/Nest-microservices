...
export class ApiGatewayAuthController {
    ...
    @Post('register')
    async register(@Body() createUserDto: UserRegisterDto): Promise<UserRegisterResponse> {
        const user = await this.apiGatewayAuthService.register(createUserDto);
    ...
    @Get('createadmin')
    async createAdmin(data: UserRegisterDto) {
        return await this.apiGatewayAuthService.registerAdmin({ email: 'admin@mail.ru', password: 'string', name: 'Name' })
    }
}