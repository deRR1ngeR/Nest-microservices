...
export class ApiGatewayAuthService {
    ...
    async register(dto: UserRegisterDto): Promise<UserRegisterResponse> {
        return await this.authService.send('register', dto).pipe(
            catchError((error) =>
    ...
    async registerAdmin(dto: UserRegisterDto): Promise<UserRegisterResponse> {
        return await this.authService.send('createAdmin', dto).pipe(
            catchError((error) =>
                throwError(() => new RpcException(error.response)),
            ),
        ).toPromise();
    ...
    async getUserByEmail(data: AccountGetUserByEmail.Request): Promise<AccountRegister.Response> {
        return await this.authService.send(AccountGetUserByEmail.topic, data.email.toString()).pipe(
            catchError((error) =>
                throwError(() => new RpcException(error.response)))
        ).toPromise();
    }
